import type { ReviewDirection } from "./srs";

export type ReviewMode = ReviewDirection;
export type ModeFilter = "mixed" | "ko-meaning" | "meaning-ko" | "sentence";

export const MODE_FILTERS: ModeFilter[] = ["mixed", "ko-meaning", "meaning-ko", "sentence"];
...
export function availableModes(word: ReviewWord, filter: ModeFilter): ReviewMode[] {
  ...
  const mixed: ReviewMode[] = ["ko-meaning", "meaning-ko"];
  if (hasSentence(word)) mixed.push("sentence-meaning", "sentence-completion");
  return mixed;
}
...
export function assignQueueModes<T extends ReviewWord>(
  ...
  const rotation: ReviewMode[] = ["ko-meaning", "meaning-ko", "sentence-meaning", "sentence-completion"];
  let cursor = 0;
